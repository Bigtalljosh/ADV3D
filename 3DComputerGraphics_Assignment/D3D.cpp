...
D3D::D3D()
{
	...
	m_rasterStateWireframeON = 0;
	m_rasterStateWireframeOFF = 0;
						  ...
						  float screenDepth, float screenNear)
{
	...
	// Set the feature level to DirectX 10/11.
	//featureLevel = D3D_FEATURE_LEVEL_11_0;
	featureLevel = D3D_FEATURE_LEVEL_10_0;  //Needed at home not DX11 Capable
	...
	//result = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_REFERENCE, NULL, 0, &featureLevel, 1, 
		//								   D3D11_SDK_VERSION, &swapChainDesc, &m_swapChain, &m_device, NULL, &m_deviceContext); //Used for CPU(slooooooow)
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if(FAILED(result))
	{
		return false;
	...
	// Setup a raster description which turns on wireframe
	rasterDesc.AntialiasedLineEnable = false;
	rasterDesc.CullMode = D3D11_CULL_NONE;
	rasterDesc.DepthBias = 0;
	rasterDesc.DepthBiasClamp = 0.0f;
	...
	//Create the wireframe toggle on state
	result = m_device->CreateRasterizerState(&rasterDesc, &m_rasterStateWireframeON);
	if(FAILED(result))
	{
		return false;
	}

	// Setup a raster description which turns off wireframe.
	rasterDesc.AntialiasedLineEnable = false;
	rasterDesc.CullMode = D3D11_CULL_NONE;
	rasterDesc.DepthBias = 0;
	rasterDesc.DepthBiasClamp = 0.0f;
	...
	//Create the wireframe toggle off state
	result = m_device->CreateRasterizerState(&rasterDesc, &m_rasterStateWireframeOFF);
	if(FAILED(result))
	{
		return false;
	}
...
void D3D::Shutdown()
{
	...
	if(m_rasterStateWireframeON)
	{
		m_rasterStateWireframeON->Release();
		m_rasterStateWireframeON = 0;
	}

	if(m_rasterStateWireframeOFF)
	{
		m_rasterStateWireframeOFF->Release();
		m_rasterStateWireframeOFF = 0;
	}
...
//Present the back buffer to the screen showing the scene
void D3D::EndScene()
{
...
void D3D::ToggleWireFrame()
{
	...
	{
		m_deviceContext->RSSetState(m_rasterStateWireframeON);
		isWireFrame = true;
	}
	else
	{
		m_deviceContext->RSSetState(m_rasterStateWireframeOFF);
		isWireFrame = false;
	}
}